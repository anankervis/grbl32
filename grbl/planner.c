...
typedef struct
{
	int32_t position[AXIS_COUNT];           // The planner position of the tool in absolute steps. Kept separate
	                                   // from g-code position for movements requiring multiple line motions,
	                                   // i.e. arcs, canned cycles, and backlash compensation.
	                                 float previous_unit_vec[AXIS_COUNT];    // Unit vector of previous path line segment
...
void plan_update_velocity_profile_parameters()
{
	...
	plan_block_t *block;
	float nominal_speed;
	float prev_nominal_speed = FLT_MAX;  // Set high for first block nominal speed calculation.
	while(block_index != block_buffer_head)
	{
...
/* Add a new linear movement to the buffer. target[AXIS_COUNT] is the signed, absolute target position
...
uint8_t plan_buffer_line(float *target, plan_line_data_t *pl_data)
{
	...
	int32_t target_steps[AXIS_COUNT], position_steps[AXIS_COUNT];
	float unit_vec[AXIS_COUNT], delta_mm;
	uint8_t idx;
	...
	} else
	{
		...
		float junction_unit_vec[AXIS_COUNT];
		...
		} else
		{
			if (junction_cos_theta < -0.999999)
			{
				...
				block->max_junction_speed_sqr = FLT_MAX;
			}
			else
			{
