...
#endif
	uint32_t addr = coord_select*(sizeof(float)*AXIS_COUNT + 1) + EEPROM_ADDR_PARAMETERS;
	memcpy_to_eeprom_with_checksum(addr, (char*)coord_data, sizeof(float)*AXIS_COUNT);
}
...
{
	uint32_t addr = coord_select*(sizeof(float)*AXIS_COUNT + 1) + EEPROM_ADDR_PARAMETERS;
	if (!(memcpy_from_eeprom_with_checksum((char*)coord_data, addr, sizeof(float)*AXIS_COUNT)))
	{
...
uint8_t settings_store_global_setting(uint8_t parameter, float value)
{
	...
	if (parameter >= AXIS_SETTINGS_START_VAL)
	{
		...
		while (set_idx < AXIS_N_SETTINGS)
		{
			...
			{
				set_idx++;
				// If axis index greater than AXIS_COUNT or setting index greater than number of axis settings, error out.
	...
	else
	{
		...
		{
		case 0:
			if (value < STATUS_SETTING_STEP_PULSE_MIN_LIMIT)
			{
